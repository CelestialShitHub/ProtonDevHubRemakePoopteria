...
local OpenTreasureChest = game:GetService("ReplicatedStorage").network.RemoteFunction.openTreasureChest
local UseTeleporter = game:GetService("ReplicatedStorage").network.RemoteFunction.playerRequest_useTeleporter
local PickUpItem = game:GetService("ReplicatedStorage").network.RemoteFunction.pickUpItemRequest
...
if not isfile("vesteria.config") then 
    writefile("vesteria.config", tostring(http:JSONEncode({})))
end
...
local function GetSetting(name)
    ...
    local content = readfile("vesteria.config")
...
local function AddSetting(name, value)
    ...
    writefile("vesteria.config", tostring(http:JSONEncode(parsed)))
end
...
function library:Window(options)
	...
	BloxburgTitle2.BackgroundColor3 = Color3.fromRGB(255, 255, 255)
	BloxburgTitle2.BackgroundTransparency = 1.000
	BloxburgTitle2.Position = UDim2.new(0.0475, 0, 0.174, 0)
	BloxburgTitle2.Size = UDim2.new(0, 372, 0, 35)
	BloxburgTitle2.Font = Enum.Font.GothamBold
	BloxburgTitle2.Text = "Epic Thing"
	BloxburgTitle2.TextColor3 = Color3.fromRGB(233, 233, 233)
	BloxburgTitle2.TextSize = 15.000
	BloxburgTitle2.TextXAlignment = Enum.TextXAlignment.Left
...
local setthreadcontext = set_thread_context or setthreadcontext or set_thread_identity or syn and syn.set_thread_identity
local hookfunction = hookfunction or hookfunc or replaceclosure
local islclosure = islclosure or is_l_closure
local newcclosure = newcclosure or protect_closure
local getrawmetatable = getrawmetatable

local hookmetamethod = hookmetamethod or function(Table, Method, Callback)
    local MetaTable = getrawmetatable(Table)
    local OldMethod 

    local Callback = islclosure(Callback) and newcclosure(Callback) or Callback

    task.spawn(function()
        task.wait()
        OldMethod = hookfunction(MetaTable[Method], Callback)
    end)
...
--// supported exploit check 

-- only for functions that some exploits dont have, 99% of exploits have the rest

assert(hookmetamethod, "Exploit not supported!")
assert(getscriptclosure, "Exploit not supported!")
assert(setthreadcontext, "Exploit not supported!")
...
local Services = setmetatable({}, {
    __index = function(self, index)
        local CachedService = ServiceCache[index];
        
        if not CachedService then 
            ServiceCache[index] = select(2, pcall(game.GetService, game, index));
...
local PlaceFolders = workspace.placeFolders 
local EntityManifestCollection = PlaceFolders.entityManifestCollection

local Signal = ReplicatedStorage:WaitForChild("signal") 
local PlayerRequest = ReplicatedStorage:WaitForChild("playerRequest")
local JoltFunction = game:GetService("ReplicatedStorage").network.BindableEvent_Client.applyJoltVelocityToCharacter

local ActiveAbilityExecutionData = Hitbox:WaitForChild("activeAbilityExecutionData")
local ActivateAbilityRequest = game:GetService("ReplicatedStorage").network.BindableFunction_Client.activateAbilityRequest
...
local function BypassTP(Target)
	...
	if type(Target) ~= 'vector' then
		distance = (Target.Position - Hitbox.Position).Magnitude + math.random(100, 120)
	else 
		distance = (Target - Hitbox.Position).Magnitude
	end
	local requiredVelocity = Vector3.new(math.huge, -math.huge, math.huge)
	
	--removeVelocity = Hitbox.hitboxVelocity.Velocity * -1
	JoltFunction:Fire(requiredVelocity)
	Hitbox.Velocity = Vector3.new(0,0,0)
	if type(Target) ~= 'vector' then 
		Hitbox.CFrame = Target.CFrame + Vector3.new(2 + math.random(), 4 + math.random(), 0)
	else
		Hitbox.CFrame = CFrame.new(Target)
	end
	task.spawn(function()
		task.wait()
		Hitbox.Velocity = Vector3.new(0,0,0)
...
local Window = library:Window({Name = "Vesteria"})

local Tabs = {
    Local = Window:Tab({Name = "Local"}),
    General = Window:Tab({Name = "General"}),
    Combat = Window:Tab({Name = "Combat"})
    
}

local Sections = {
    Random = Tabs.General:Section({Name = "Random"}),
    Character = Tabs.Local:Section({Name = "Character"}),
    Combat = Tabs.Combat:Section({Name = "Combat"})
}
...
do
    ...
    OldIndex = hookmetamethod(game, "__index", function(self, index)
        if library.flags["Infinite Stamina"] and tostring(self) == "stamina" and index == "Value" then 
            local Parent = self.Parent

            if Parent then 
                local MaxStamina = Parent:FindFirstChild("maxStamina")

                if MaxStamina then
                    return OldIndex(MaxStamina, index)
                end 
            end
        end
    ...
    OldNamecall = hookmetamethod(game, "__namecall", function(self, ...)
        ...
        if (library.flags["Godmode"] or library.flags['Auto-Farm']) and getnamecallmethod() == "FireServer" then 
			if Arguments[1] == "replicateClientStateChanged" and type(Arguments[4]) == "number" then
				Arguments[4] = nil
				return OldNamecall(self, unpack(Arguments))
			elseif Arguments[4] == "monster" then
				return
			end 
        end
...
do 
    Player.CharacterAdded:Connect(function(Character)
        ...
        ActiveAbilityExecutionData.Changed:Connect(function(Property)
            --if library.flags.UseAbilities then
...
for Index, Value in next, getgc() do
	if type(Value) == "function" and getfenv(Value).script == LocalPlayer.PlayerScripts.repo.controlScript and debug.getinfo(Value).name == 'onPlayerStatisticsChanged' then
		WalkspeedTable = Value
	end
end


do
    Sections.Character:Toggle({Name = "Godmode"})
    Sections.Character:Toggle({Name = "Flight"})
    Sections.Character:Slider({Name = "Flight Speed", Min = 1, Max = 1000, Default = 200})
    Sections.Character:Slider({Name = "Walkspeed", Min = 10, Max = 500, Default = 10, Callback = function(Value)
		debug.setupvalue(WalkspeedTable, 2, Value)
	end})
end
...
local function fly()
    ...
    local velocity = (CurrentCameraCFrame.rightVector * LeftRight + CurrentCameraCFrame.lookVector * FowardBack) * library.flags['Flight Speed']
...
for Index, Value in next, workspace:GetDescendants() do 
    if Value.ClassName == "Model" and Value.Name:match("%w+Chest") and Value.Name:sub(Value.Name:len() - 4, Value.Name:len()) == "Chest" then 
...
for Index, Value in next, workspace:GetDescendants() do 
	if Value.Name == 'teleportDestination' then 
		local Name = "ERROR"
		local success, errVal = pcall(function()
			Name = MarketplaceService:GetProductInfo(Value.Value).Name
		end)
		if not success then 
			LocalPlayer:Kick("429 ERROR, too many requests to ROBLOX services. Please disable your VPN or wait before re-executing the script.")
		end
		if Name ~= 'ERROR' and ActualTeleports[Name] == nil then 
...
do
    Sections.Random:Toggle({Name = "Infinite Stamina"})
	Sections.Random:Toggle({Name = "Auto-Pickup"})
	Sections.Random:SearchBox({Name = "Place Teleports", List = Teleports, DontSave = true, Callback = function(Value)
		PlayerRequest:InvokeServer("playerRequest_useTeleporter", ActualTeleports[Value])
	end})
	Sections.Random:Button({Name = "Chest-Farm", Callback = Chestfarm})
end

local lastPickupCall = os.time()
local amountPickedUp = 0
local function AutoPickup()
    if not Hitbox then
        return 
    end
	if os.time() - lastPickupCall >= 1 then
		amountPickedUp = 0
		lastPickupCall = os.time()
		for Index, Value in next, PlaceFolders.items:GetChildren() do 
			if Value:FindFirstChild("canPickup") and Value.canPickup.Value and amountPickedUp < 10 then 
				task.spawn(function()
					PlayerRequest:InvokeServer("pickUpItemRequest", Value)
					amountPickedUp += 1
				end)
			end
		end
...
do 
    Sections.Combat:Toggle({Name = "Kill Aura"})
	Sections.Combat:Toggle({Name = "Auto-Farm"})
end

local function GetAttackable(MaxDistance)
    ...
    if not Attacking then 
        for Index, Value in next, EntityManifestCollection:GetChildren() do 
...
local function GetClosest()
    ...
    for Index, Value in next, GetAttackable(math.huge) do 
		local Magnitude = (Value[2] - Hitbox.Position).Magnitude
    ...
    repeat 
        if Hitbox then
            if library.flags['Auto-Farm']  then 
				if autofarmTarget == nil or autofarmTarget:FindFirstChild("health") == nil or autofarmTarget.health.Value <= 0 then 
					autofarmTarget = Enemy
                	BypassTP(Enemy) 
				else
					BypassTP(autofarmTarget)
				end 
            end
			lastswing = tick()
			...
			for _ = 1 , 2 ,1 do
				atkCounter = math.max(1, (atkCounter + 1) % 4)
				Signal:fireServer('replicatePlayerAnimationSequence', 'dualAnimations', 'strike' .. atkCounter, { attackSpeed = -1 })
				Signal:FireServer("playerRequest_damageEntity_batch", GetAttackable(AttackingPlayers))
				wait()
			end
        ...
        task.wait()
    until not Enemy:FindFirstChild("health") or Enemy.health.Value <= 0 or not Hitbox or not library.flags['Auto-Farm']
    
end

getgenv().running = true
while task.wait() and getgenv().running do
    if library.flags['Kill Aura'] or library.flags['Auto-Farm'] then 
	...
	--BypassTP(Vector3.new(math.random(100,2000),math.random(100,2000),math.random(100,2000)))
	Hitbox.Anchored = false
end
